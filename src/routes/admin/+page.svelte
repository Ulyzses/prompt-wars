<script lang="ts">
  ...
  const setTimer = (s: number) => new Promise((resolve, reject) => {
  ...
  const timeToString = (s: number) => `${s/60 | 0}:${String(s % 60).padStart(2, '0')}`;
...
<form on:submit|preventDefault>
  {#if $session}
    <p>Current Session: [{$session.id}] {$session.name} ({$session.state} Round {$session.round} of {$session. max_rounds})</p>
    <p>Players</p>
