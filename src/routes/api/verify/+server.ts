...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (password === playerQuery.data.password) {
    ...
    if (!existingBreaks.some((row) => row.attacker === attacker)) {
      ...
      const insertBreaksQuery = await supabase.from("breaks").insert({
        session: session.id,
        round: session.round,
        defender,
        attacker,
      ...
      if (insertBreaksQuery.error) {
        ...
        return json(
          { correct: true, error: `${code}: ${message}` },
          { status: 500 },
        );
      }

      const updateAttackerQuery = await supabase.rpc("update_score", {
        val: score,
        player_id: attacker,
      });
      console.log("Attacker", updateAttackerQuery);

      const updateDefenderQuery = await supabase.rpc("update_score", {
        val: 0 - score,
        player_id: defender,
      });
